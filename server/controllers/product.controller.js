...
const getProductsByName = async (req, res) => {
  ...
  try {
    if (!name) {
      res
        .status(404)
        .send({ success: false, error: "Product name is required" });
    } else {
      const exist = await findDocument(
        process.env.APPWRITE_PRODUCT_COLLECTION_ID,
        "name",
...
const getProductsByType = async (req, res) => {
  ...
  try {
    if (!type) {
      res
        .status(404)
        .send({ success: false, error: "Product type is required" });
    } else {
      const exist = await findDocument(
        process.env.APPWRITE_PRODUCT_COLLECTION_ID,
        "type",
  ...
  try {
    if (!name || !type || !price) {
      ...
      if (exist.response.total != 0) {
        res
          .status(404)
          .send({ success: false, error: "This product is already present" });
      } else {
        const productData = {
          name,
          type,
        ...
        const createProductResponse = await createDocument(
          process.env.APPWRITE_PRODUCT_COLLECTION_ID,
          productData
        );
...
const deleteProduct = async (req, res) => {
  ...
  try {
    if (!name || !type) {
      ...
      if (exist.response.total != 0) {
        const deleteProductResponse = await deleteDocument(
          process.env.APPWRITE_PRODUCT_COLLECTION_ID,
  ...
  try {
    if (!name || !type || !newPrice) {
      ...
      if (exist.response.total != 0) {
        ...
        const updatePriceResponse = await updateSingleDocument(
          process.env.APPWRITE_PRODUCT_COLLECTION_ID,
          exist.response.documents[0].$id,
          updates
        );
